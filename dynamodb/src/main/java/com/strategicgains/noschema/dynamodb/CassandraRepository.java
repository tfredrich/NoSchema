...
import com.strategicgains.noschema.dynamodb.DocumentSchemaProvider.Columns;
import com.strategicgains.noschema.dynamodb.key.KeyDefinition;
import com.strategicgains.noschema.dynamodb.key.KeyPropertyConverter;
...
implements NoSchemaRepository<T>
{
	...
	private Map<String, DynamodbDocumentFactory<T>> factoriesByView = new HashMap<>();
	...
	public CassandraRepository(CqlSession session, Table table, View... views)
	{
		...
		if (views != null && views.length > 0)
		{
			...
			for(View view : views)
			{
				...
				this.factoriesByView.put(view.name(), new DynamodbDocumentFactory<>(view.keys()));
			}
		}
	}
