...
implements StatementFactory<T>
{
	...
	private DynamodbDocumentFactory<T> documentFactory;

	public DocumentStatementFactory(CqlSession session, Table table)
	{
		this(session, table, new DynamodbDocumentFactory<>(table.keys()));
	}

	public DocumentStatementFactory(CqlSession session, Table table, DynamodbDocumentFactory<T> factory)
	{
		super();
